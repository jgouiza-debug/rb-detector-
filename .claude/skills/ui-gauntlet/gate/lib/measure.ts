...
export interface PageAudit {
  ...
  /**
   * Controls that actually take you somewhere else: a link to another path, or
   * a form submit. A screen is a dead end when it has none of these, however
   * many buttons it has — and it is NOT a dead end merely for having one, which
   * a terminal confirmation screen legitimately does.
   */
  exitCount: number;
  headingCount: number;
}
...
export function auditPage(opts: {
  primaryBox: { x: number; y: number; width: number; height: number } | null;
  viewportW: number;
  viewportH: number;
}): PageAudit {
  ...
  function effectiveBackground(
    el: Element,
  ): { r: number; g: number; b: number } | "unknown" {
  ...
  const interactiveSel =
    "a[href], button, input, select, textarea, [role='button'], [role='tab'], [role='checkbox'], [role='radio'], [role='switch'], [tabindex]:not([tabindex='-1'])";
  const targets: TargetInfo[] = [];
  for (const el of all) {
    if (!el.matches(interactiveSel)) continue;
    if (el.matches("input[type='file'][hidden], input[hidden]")) continue;
    const cs = getComputedStyle(el);
    if (cs.position === "absolute" && cs.left && cs.left.startsWith("-"))
      continue; // skip-link style off-screen
  ...
  for (const el of all) {
    ...
    const isBox =
      el.matches("img, svg, input, textarea, button, a[href], hr") ||
    ...
    if (rb.top < vh && rb.bottom <= vh + 1)
      lastContentBottom = Math.max(lastContentBottom, Math.round(rb.bottom));
  }
  const scrolls =
    document.documentElement.scrollHeight > window.innerHeight + 4;
  const composition = {
    bottomVoid: scrolls ? 0 : Math.max(0, vh - lastContentBottom),
    lastContentBottom,
    scrolls,
  };
  ...
  if (overflowPx > 0) {
    for (const el of all) {
      ...
      if (r2.width > vw + 1 && overflowOffenders.length < 5)
        overflowOffenders.push({ el: describe(el), w: Math.round(r2.width) });
    }
  }
  ...
  for (const el of all) {
    ...
    for (const prop of list) {
      ...
      if (
        ...
        autoClass(el.className, prop)
      )
        continue;
      spacingCounts.set(v, (spacingCounts.get(v) || 0) + 1);
      if (v >= 8 && v % 8 !== 0 && offenders.length < 40)
        offenders.push({ el: describe(el), prop, value: v });
    }
  }
  ...
  for (const parent of all) {
    ...
    for (const child of Array.from(parent.children)) {
      ...
      const cls =
        typeof child.className === "string" ? child.className.split(/\s+/) : [];
      if (
        cls.includes("mx-auto") ||
        cls.includes("self-center") ||
  ...
  for (const el of textEls) {
    ...
    textColorCounts.set(
      toHex(fgEff),
      (textColorCounts.get(toHex(fgEff)) || 0) + 1,
    );
  ...
  const sortMap = <K>(m: Map<K, number>) =>
  ...
  return {
    ...
    spacing: {
      ...
      offGrid,
      offenders,
      distinctLeftEdges: Array.from(leftEdges).sort((a, b) => a - b),
    },
    type: {
      sizes: sortMap(sizeCounts),
      families: sortMap(familyCounts),
      weights: sortMap(weightCounts),
    },
    color: {
      textColors: sortMap(textColorCounts),
      contrastFailures,
      contrastChecked,
      contrastUnknown,
    },
    ...
    exitCount: Array.from(
      document.querySelectorAll<HTMLElement>(
        "a[href], button[type='submit'], form",
      ),
    ).filter((el) => {
      const href = el.getAttribute("href");
      if (href === null) return true; // a submit or a form leaves by posting
...
export function probeFocus(): FocusProbe | null {
  ...
  return {
    el: `${tag}${label ? `[aria-label="${label}"]` : ""}${text ? ` "${text}"` : ""}`,
 ...
 */
export function probeSticky(
  before: { el: string; top: number }[] | null,
): StickyProbe[] | { el: string; top: number }[] {
