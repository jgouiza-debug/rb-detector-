...
export default function SignInPage() {
  ...
  async function sendCode() {
    setBusy(true);
    setErr(null);
    await fetch("/api/auth/signin/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email }) });
    setStage("code");
    setBusy(false);
  }
  async function verify() {
    setBusy(true);
    setErr(null);
    const res = await fetch("/api/auth/signin/verify", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email, code }) });
