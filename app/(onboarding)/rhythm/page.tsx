    ...
    >
      ...
      <div className="flex flex-col gap-3">
        <label className="flex items-center justify-between rounded-2xl border border-line bg-surface px-4 py-3">
          ...
          <input type="time" value={morning} onChange={(e) => setMorning(e.target.value)} aria-label="morning check-in time" className="bg-transparent text-lg text-fg outline-none" />
        </label>
        <label className="flex items-center justify-between rounded-2xl border border-line bg-surface px-4 py-3">
          <span className="font-semibold">evening</span>
          <input type="time" value={evening} onChange={(e) => setEvening(e.target.value)} aria-label="evening check-in time" className="bg-transparent text-lg text-fg outline-none" />
        </label>
      </div>
    </StepShell>
  );
