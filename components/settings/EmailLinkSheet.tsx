...
export function EmailLinkSheet() {
  ...
  return (
    <Sheet open={open} onClose={() => setOpen(false)} title="keep pip+ on any device" dismissible={false}>
      ...
      {stage === "email" ? (
        <>
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@email.com" aria-label="email" className="mb-4 w-full rounded-field border border-line bg-surface px-4 py-2 min-h-12 outline-none focus-visible:outline-3 focus-visible:outline-ring" />
          <Button full onClick={send} disabled={busy || !email.includes("@")}>send me a code</Button>
        </>
      ) : (
        <>
          <input inputMode="numeric" value={code} onChange={(e) => setCode(e.target.value)} placeholder="000000" aria-label="code" className="mb-4 w-full rounded-field border border-line bg-surface px-4 py-2 min-h-12 text-center text-lg tracking-[0.3em] outline-none focus-visible:outline-3 focus-visible:outline-ring" />
