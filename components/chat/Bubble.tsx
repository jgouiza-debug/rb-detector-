  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {isVoice && (
            <Icon icon={Mic} size={16} className="shrink-0 text-fg-soft" />
          )}
          <span>{message.text}</span>
        </div>
        {isVoice && (
          <span className="mt-1 px-1 text-xs text-fg-soft">
            noted quietly · no reply
          </span>
        )}
        {message.status === "failed" && (
          <button
            type="button"
            onClick={() =>
              retry((message.meta.clientId as string) ?? message.id)
            }
        ...
        {showTime && (
          <span className="mt-1 px-1 text-xs text-fg-soft">{time}</span>
        )}
      </div>
    </div>
  );
