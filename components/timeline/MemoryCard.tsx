  ...
  return (
    ...
    >
      <div className="p-6">
        ...
        {day.title ? (
          <>
            <h2 className="font-reading text-2xl leading-tight text-fg">
              {day.title}
            </h2>
        ...
        {pending ? (
          <div className="mt-4 space-y-2">
            <p className="text-sm italic text-fg-soft">
              pip is wrapping up{" "}
              {formatLongDate(day.date).split(",")[0].toLowerCase()}…
            </p>
            <Skeleton className="h-4 w-full" />
            <Skeleton className="h-4 w-4/5" />
          </div>
        ) : (
          <p className="font-reading mt-4 text-lg leading-relaxed text-fg">
            {day.reflection}
          </p>
        )}
        {day.thumbs.length > 0 && (
          <div className="mt-4 grid grid-cols-2 gap-2">
            {day.thumbs.slice(0, 2).map((t) => (
              ...
              <img
                key={t.id}
                src={`/api/media/${t.id}?v=thumb`}
                alt={t.caption ?? "a photo from this day"}
                className={`h-32 w-full rounded-card object-cover ${day.thumbs.length === 1 ? "col-span-2" : ""}`}
              />
            ))}
          </div>
        )}
      ...
      <div className="flex items-center justify-between border-t border-line py-2 pl-6 pr-4">
        ...
        <Link
          ...
          className="tap inline-flex items-center gap-1 rounded-pill px-2 text-sm font-semibold text-fg transition-colors duration-150 hover:bg-surface-2 active:bg-line/40"
        >
