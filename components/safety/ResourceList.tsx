...
export function ResourceRows({
  ...
  ground?: "surface" | "surface-2";
}) {
  return (
    <ul
      className={`flex flex-col divide-y divide-line rounded-card ${ground === "surface" ? "bg-surface" : "bg-surface-2"}`}
    >
      {resources.map((r) => {
          ...
          <li key={`${r.region}-${r.name}`}>
            ...
            >
              <span className="min-w-0 flex-1">
                <span className="block text-xs font-semibold tracking-wide text-fg-soft">
                  {r.region}
                </span>
                <span className="block text-sm font-semibold">
                  {r.name}{" "}
                  <span className="font-normal text-fg-soft">· {r.detail}</span>
                </span>
              </span>
              <Icon
                icon={r.tel ? Phone : r.sms ? MessageSquare : ExternalLink}
                size={18}
...
export function PrimaryResource({
  ...
  return (
    <div>
      <div className="mb-2 text-xs font-semibold tracking-wide text-fg-soft">
      ...
      <div className="flex gap-2">
        {resource.tel && (
          <a
            ...
            className="tap crisis-primary inline-flex h-14 flex-1 items-center justify-center gap-2 rounded-pill bg-cta text-lg font-semibold text-cta-fg transition-transform duration-150 active:scale-[0.98]"
          >
        ...
        {resource.sms && (
          <a
            ...
            className={`tap inline-flex h-14 flex-1 items-center justify-center gap-2 rounded-pill ${ground === "surface" ? "bg-surface" : "bg-surface-2"} text-lg font-semibold text-fg ring-1 ring-line transition-transform duration-150 active:scale-[0.98]`}
          >
