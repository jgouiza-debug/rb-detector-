...
export function BackLink({ href, children, className }: { href: string; children: ReactNode; className?: string }) {
  return (
    <Link href={href} className={cn("tap -ml-2 inline-flex w-fit items-center gap-1 self-start rounded-pill px-2 text-sm font-semibold text-fg-soft transition-colors duration-150 hover:bg-surface-2 active:bg-line/40", className)}>
