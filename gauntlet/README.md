...
| A4 | Distinct font sizes/families/weights across the app, pixel-coverage colour histogram of each viewport (70/20/10 proxy: near-neutrals such as cream + white cards count as one family, changed after round 0; regions covered by `<img>`/`<video>` are excluded, changed in round 7 after a screen failed the palette gate for the crime of displaying a user photograph) |
