...
function attachConsole(page: Page, current: { screen: string }, sink: ConsoleEvent[]) {
  page.on("console", (msg) => {
    const type = msg.type();
    if (type !== "error" && type !== "warning") return;
    sink.push({ screen: current.screen, kind: type === "error" ? "console.error" : "console.warning", text: msg.text() });
  });
  page.on("pageerror", (err) => sink.push({ screen: current.screen, kind: "pageerror", text: err.message }));
  page.on("response", (res) => {
    ...
    sink.push({ screen: current.screen, kind: "http", text: `${status} ${res.request().method()} ${url.pathname}`, expected });
  });
}
...
async function captureScreen(page: Page, s: Screen, theme: "light" | "dark", events: ConsoleEvent[], probeFocusRing: boolean): Promise<ScreenResult> {
  ...
  const shots: ScreenResult["shots"] = { viewport: path.relative(ROUND_DIR, viewportShot) };
  if (s.fullPage) {
  ...
  const violations: AxeViolation[] = axe.violations.map((v) => ({ id: v.id, impact: v.impact ?? "unknown", tags: v.tags, nodes: v.nodes.length, help: v.help, sample: v.nodes.slice(0, 2).map((n) => n.html) }));
  const wcagSeriousOrCritical = violations.filter((v) => (v.impact === "serious" || v.impact === "critical") && v.tags.some((t) => t.startsWith("wcag"))).length;
...
async function runTheme(page: Page, theme: "light" | "dark") {
  ...
  if (theme === "dark") await page.addInitScript(() => localStorage.setItem("pip-theme", "dark"));
  ...
  for (const s of SCREENS) {
    ...
    results.push(await captureScreen(page, s, theme, events, theme === "light"));
  }
}
...
test.describe("reduced motion", () => {
  ...
  test("no animation keeps running when the user asks for less motion", async ({ page }) => {
    await page.goto("/welcome");
    await page.request.post("/api/dev/reset");
    await page.context().clearCookies();
    const walk: Screen[] = SCREENS.filter((s) => ["onboarding-welcome", "onboarding-name", "onboarding-focus", "onboarding-rhythm", "onboarding-notify", "thread", "timeline", "meditation-pause"].includes(s.id));
    for (const s of walk) {
      if (s.setup) await s.setup(page);
      ...
      reduced.push({ screen: s.id, runningAnimations: r.running, longestMs: r.longest });
    }
  });
});

test.afterAll(async () => {
  if (results.length === 0) throw new Error("no screens were captured; refusing to write an empty gate report");
  const report = evaluate(ROUND, VIEWPORT, results, reduced);
  fs.writeFileSync(path.join(ROUND_DIR, "gate-report.json"), JSON.stringify(report, null, 2));
  fs.writeFileSync(path.join(ROUND_DIR, "gate-report.md"), renderMarkdown(report));
  console.log(`\nGate report → ${path.relative(process.cwd(), path.join(ROUND_DIR, "gate-report.md"))}`);
  console.log(`A1 ${report.tiers.A1} · A2 ${report.tiers.A2} · A3 ${report.tiers.A3} · A4 ${report.tiers.A4} · A5 ${report.tiers.A5}`);
});
