 ...
 */
export async function colorCoverage(
  png: Buffer,
  topN = 8,
  ...
  for (let i = 0; i < data.length; i += 3) {
    ...
    if (boxes.some((b) => x >= b.x0 && x < b.x1 && y >= b.y0 && y < b.y1))
      continue;
    px++;
    const key =
...
export function evaluate(
  ...
  // ── A1 accessibility ─────────────────────────────────────────────────
  const axeSerious = screens.reduce(
    (n, s) => n + s.axe.wcagSeriousOrCritical,
    0,
  );
  const axeContrast = screens.reduce((n, s) => n + s.axe.contrast, 0);
  const measuredContrast = screens.reduce(
    (n, s) => n + s.audit.color.contrastFailures.length,
    0,
  );
  ...
  checks.push({
    id: "A1.axe",
    label: "axe-core WCAG 2.x A/AA serious+critical violations",
    value: String(axeSerious),
    pass: axeSerious === 0,
    detail: screens
      .filter((s) => s.axe.wcagSeriousOrCritical > 0)
      .map(
        (s) =>
          `${s.id}/${s.theme}: ${s.axe.violations
            .filter(
              (v) =>
  ...
  checks.push({
    id: "A1.contrast",
    label: "Rendered text contrast failures (axe + measured)",
    value: `${axeContrast} axe / ${measuredContrast} measured`,
    pass: axeContrast === 0 && measuredContrast === 0,
    detail: screens
      .flatMap((s) =>
        s.audit.color.contrastFailures
          .slice(0, 3)
          .map(
            (f) =>
              `${s.id}/${s.theme}: ${f.el} ${f.fg} on ${f.bg} = ${f.ratio}:1 (needs ${f.required})`,
          ),
      )
      .join("; "),
  });
  checks.push({
    id: "A1.focus",
    label: "Keyboard focus indicator visible on tabbed controls",
    value: `${focusProbes.length - focusMissing.length}/${focusProbes.length}`,
    pass: focusMissing.length === 0,
    detail: focusMissing
      .slice(0, 5)
      .map((f) => f.el)
      .join("; "),
  ...
  checks.push({
    id: "A1.reducedMotion",
    label: "Animations still running under prefers-reduced-motion",
    value: String(rmRunning),
    pass: rmRunning === 0,
    detail: reducedMotion
      .filter((r) => r.runningAnimations > 0)
      .map(
        (r) => `${r.screen}: ${r.runningAnimations} (longest ${r.longestMs}ms)`,
      )
      .join("; "),
  });
  const A1 =
  ...
  // ── A2 touch + ergonomics ────────────────────────────────────────────
  const allTargets = screens.flatMap((s) =>
    s.audit.targets.map((t) => ({ ...t, screen: `${s.id}/${s.theme}` })),
  );
  ...
  const primaries = screens.map((s) => ({
    screen: `${s.id}/${s.theme}`,
    p: s.audit.primary,
  }));
  const primaryMissing = primaries.filter((x) => !x.p || !x.p.found);
  const primarySmall = primaries.filter(
    (x) => x.p && x.p.found && (x.p.w < 44 || x.p.h < 44),
  );
  const primaryBelowFold = primaries.filter(
    (x) => x.p && x.p.found && !x.p.inViewport,
  );
  const primaryOutOfZone = primaries.filter(
    (x) =>
      x.p &&
      x.p.found &&
      x.p.inViewport &&
      !x.p.inThumbZone &&
      screens.find((s) => `${s.id}/${s.theme}` === x.screen)?.thumbZone,
  );
  checks.push({
    id: "A2.targets",
    label: "Interactive targets under 44×44 (block-level)",
    value: `${smallBlock.length}/${blockTargets.length}`,
    pass: smallBlock.length === 0,
    detail: Array.from(
      new Set(smallBlock.map((t) => `${t.screen}: ${t.el} ${t.w}×${t.h}`)),
    )
      .slice(0, 12)
      .join("; "),
  });
  checks.push({
    id: "A2.inline",
    label: "Inline text links under 44px (WCAG 2.5.8 exempt, advisory)",
    value: String(smallInline.length),
    pass: true,
    detail: Array.from(
      new Set(smallInline.map((t) => `${t.screen}: ${t.el} ${t.w}×${t.h}`)),
    )
      .slice(0, 8)
      .join("; "),
  });
  const primaryBad = new Set(
    [
      ...primaryMissing,
      ...primarySmall,
      ...primaryBelowFold,
  ...
  checks.push({
    id: "A2.primary",
    label:
      "Primary action present, ≥44px, on screen at landing, in the thumb zone (CTAs)",
    value: `${primaries.length - primaryBad.size}/${primaries.length}`,
    pass: primaryBad.size === 0,
    detail: [
      ...primaryMissing.map((x) => `${x.screen}: not found`),
      ...primarySmall.map((x) => `${x.screen}: ${x.p!.w}×${x.p!.h}`),
      ...primaryBelowFold.map(
        (x) => `${x.screen}: off-screen at landing (y=${x.p!.y})`,
      ),
      ...primaryOutOfZone.map(
        (x) => `${x.screen}: centre y=${x.p!.y + x.p!.h / 2} above thumb zone`,
      ),
    ].join("; "),
  });
  ...
  for (const s of screens)
    for (const v of s.audit.spacing.values)
      if (!v.on8)
        offenderValues.set(
          v.value,
          (offenderValues.get(v.value) || 0) + v.count,
        );
  const worstEdges = Math.max(
    0,
    ...screens.map((s) => s.audit.spacing.distinctLeftEdges.length),
  );
  checks.push({
    id: "A3.grid8",
    label: "Spacing values on the 8pt grid",
    value: pct(spOn8, spTotal),
    pass: on8Share >= 0.9,
    detail: `off-8 values (count): ${Array.from(offenderValues.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 8)
      .map(([v, c]) => `${v}px×${c}`)
      .join(", ")}`,
  ...
  checks.push({
    id: "A3.grid4",
    label: "Spacing values off even the 4pt half-grid",
    value: String(spOff),
    pass: spOff === 0,
  });
  ...
  checks.push({
    id: "A3.void",
    label:
      "Empty ground below the last content on a non-scrolling screen (advisory, >200px)",
    value: `${voids.length}/${screens.length} screens`,
    pass: true,
    detail: screens
      .filter((s) => s.audit.composition.bottomVoid > 0)
      .map((s) => `${s.id}/${s.theme}: ${s.audit.composition.bottomVoid}px`)
      .join("; "),
  });
  checks.push({
    id: "A3.align",
    label:
      "Misaligned sibling blocks (distinct left edges among wide siblings, per screen max)",
    value: String(worstEdges),
    pass: worstEdges === 0,
    detail: screens
      .filter((s) => s.audit.spacing.distinctLeftEdges.length > 0)
      .map(
        (s) =>
          `${s.id}/${s.theme}: ${s.audit.spacing.distinctLeftEdges.join(",")}`,
      )
      .join("; "),
  });
  ...
  checks.push({
    id: "A4.families",
    label: `Distinct font families (≤ ${FAMILY_LIMIT})`,
    value: `${famList.length}: ${famList.join(", ")}`,
    pass: famList.length <= FAMILY_LIMIT,
  });
  checks.push({
    id: "A4.weights",
  ...
  checks.push({
    id: "A4.palette",
    label:
      "70/20/10 proxy: neutral family ≥ 55% and neutral + two accents ≥ 85% of pixels",
    value: `${screens.length - new Set([...dominantLow, ...busy]).size}/${screens.length} screens`,
    pass: dominantLow.length === 0 && busy.length === 0,
    detail: [
      ...dominantLow.map(
        (s) => `${s.id}/${s.theme}: dominant ${s.colors.dominant}%`,
      ),
      ...busy.map((s) => `${s.id}/${s.theme}: top-3 ${s.colors.top3}%`),
    ].join("; "),
  });
  const A4 =
    sizeList.length <= 6 &&
  ...
  const errors = screens.flatMap((s) =>
    s.console.filter((c) => !c.expected && c.kind !== "console.warning"),
  );
  ...
  checks.push({
    id: "A5.console",
    label: "Console errors / page errors / failed requests",
    value: String(errors.length),
    pass: errors.length === 0,
    detail: errors
      .slice(0, 8)
      .map((e) => `${e.screen}: [${e.kind}] ${e.text.slice(0, 120)}`)
      .join("; "),
  });
  checks.push({
    id: "A5.warnings",
    label: "Console warnings (advisory)",
    value: String(warnings.length),
    pass: true,
    detail: Array.from(new Set(warnings.map((w) => w.text.slice(0, 100))))
      .slice(0, 5)
      .join("; "),
  });
  checks.push({
  ...
  checks.push({
    id: "A5.deadEnds",
    label: "Dead-end screens (no primary action or < 2 controls)",
    value: String(deadEnds.length),
    pass: deadEnds.length === 0,
    detail: deadEnds.map((s) => `${s.id}/${s.theme}`).join("; "),
  });
  const stuck = screens.flatMap((s) =>
    s.sticky
      .filter((p) => !p.stayed)
      .map(
        (p) =>
          `${s.id}/${s.theme}: ${p.el} (${p.position}) left the viewport at top=${p.topAfter}`,
      ),
  );
  checks.push({
    id: "A5.sticky",
    label: "Sticky/fixed chrome still on screen after scrolling",
    value: `${screens.reduce((n, s) => n + s.sticky.filter((p) => p.stayed).length, 0)}/${screens.reduce((n, s) => n + s.sticky.length, 0)}`,
    pass: stuck.length === 0,
    detail: stuck.slice(0, 8).join("; "),
  });
  checks.push({
    id: "A5.overflow",
    label: "Screens that scroll sideways (horizontal overflow)",
    value: String(overflow.length),
    pass: overflow.length === 0,
    detail: overflow
      .map(
        (s) =>
          `${s.id}/${s.theme}: +${s.audit.overflowX.px}px (${s.audit.overflowX.offenders[0]?.el ?? "?"} ${s.audit.overflowX.offenders[0]?.w ?? "?"}px wide)`,
      )
      .join("; "),
  });
  const A5 =
  ...
  return {
    ...
    caps: {
      ...
      primaryTarget:
        primarySmall.length > 0 ||
        primaryOutOfZone.length > 0 ||
        primaryBelowFold.length > 0,
      brokenState:
        errors.length > 0 ||
        deadEnds.length > 0 ||
        overflow.length > 0 ||
...
export function renderMarkdown(r: GateReport): string {
  ...
  lines.push("");
  lines.push(
    `Generated ${r.generatedAt} · viewport ${r.viewport.width}×${r.viewport.height} · ${r.screens.length} screen renders (${r.screens.filter((s) => s.theme === "dark").length} dark)`,
  );
  lines.push("");
  ...
  lines.push("");
  lines.push(
    `Hard caps triggered: contrast→70: **${r.caps.contrast ? "YES" : "no"}** · primary target→80: **${r.caps.primaryTarget ? "YES" : "no"}** · broken state→75: **${r.caps.brokenState ? "YES" : "no"}**`,
  );
  lines.push("");
  lines.push("## Checks");
  lines.push("");
  ...
  for (const c of r.checks)
    lines.push(
      `| ${c.id} | ${c.label} | ${c.value} | ${mark(c.pass)} | ${(c.detail || "").replace(/\|/g, "\\|")} |`,
    );
  lines.push("");
  ...
  lines.push("");
  lines.push(
    `- Sizes (${r.system.sizes.length}): ${r.system.sizes.join(", ")}px`,
  );
  lines.push(
    `- Families (${r.system.families.length}): ${r.system.families.join(", ")}`,
  );
  lines.push(
    `- Weights (${r.system.weights.length}): ${r.system.weights.join(", ")}`,
  );
  lines.push("");
  lines.push(
  ...
  for (const s of r.screens)
    lines.push(
      `- ${s.id}/${s.theme}: ${s.colors.top
        .slice(0, 5)
        .map((c) => `${c.token ?? c.hex} ${c.share}%`)
        .join(" · ")}`,
    );
  lines.push("");
  ...
  for (const s of r.screens) {
    for (const v of s.axe.violations) {
      ...
      seen.add(key);
      lines.push(
        `- ${s.id}/${s.theme}: **${v.id}** (${v.impact}, ${v.nodes} node${v.nodes === 1 ? "" : "s"}) — ${v.help}${v.sample.length ? ` · e.g. \`${v.sample[0].slice(0, 100)}\`` : ""}`,
      );
    }
  }
  ...
  for (const e of evs.slice(0, 40))
    lines.push(
      `- ${e.screen}: [${e.kind}${e.expected ? ", expected" : ""}] ${e.text.slice(0, 160)}`,
    );
  lines.push("");
  lines.push("## Reduced motion");
  lines.push("");
  for (const rm of r.reducedMotion)
    lines.push(
      `- ${rm.screen}: ${rm.runningAnimations} running animation${rm.runningAnimations === 1 ? "" : "s"}${rm.runningAnimations ? ` (longest ${rm.longestMs}ms)` : ""}`,
    );
  lines.push("");
  return lines.join("\n");
}
